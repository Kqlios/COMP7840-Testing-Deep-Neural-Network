...
class WhiteBoxFuzzer:
  ...
  def start(self, hours=0, minutes=0, seconds=0, append='meta', verbose=0, NClimit = 0):
    ...
    try:
      while True:
        ...
        while len(worklist) > 0:
          ...
          # originalImg = np.reshape(input, (28, 28))
          originalImg = np.reshape(np.array(input), (32, 32, 3))
          # Select neurons.
          neurons = self.strategy(k=self.k, input=input, groundT=groundTruth)
          ...
          for _ in range(self.trail):
            ...
            with tf.GradientTape() as t:
              ...
              #   print(internals[li][ni])
              # print("sums: ")
              # print(K.sum([internals[li][ni] for li, ni in neurons]))
              # print("ground truth: ")
              # print(logits[orig_index])
              # print("loss: ")
              # print(loss)
            ...
            if label != self.label:
              # generatedImg = np.reshape(np.array(input), (32, 32, 3))
              generatedImg = np.reshape(input, (28, 28))
              ssim_value = ssim(originalImg, generatedImg, data_range=1.0)
              # ssim_value = ssim(originalImg, generatedImg, data_range=1.0, channel_axis=2, win_size=5)
              mse_value = calculate_mse(originalImg, generatedImg)
              # ssim_value = np.array(input)
            else:
              ssim_value = 2
              mse_value = -1
            ...
            # Add created input.
            count = count + 1
            self.archive.add(input, label, distance, timer.elapsed.total_seconds(), new_cov, ssim_value, mse_value)
